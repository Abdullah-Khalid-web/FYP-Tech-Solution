...
exports.register = async (req, res) => {
  ...
  if (ownerPassword !== confirmPassword) {
    // Get plans again for the dropdown
    const [plans] = await pool.execute(
      `SELECT 
         BIN_TO_UUID(id) AS id,
         name,
  ...
  try {
    ...
    // Determine logo path (if uploaded) - STORE THE CORRECT PATH
    let logo = null;
    if (req.file) {
      // Store the path as "uploads/shop_logos/filename.png"
    ...
    console.error(err);
    // Get plans again for the error view
    const [plans] = await pool.execute(
      `SELECT 
         BIN_TO_UUID(id) AS id,
         name,
